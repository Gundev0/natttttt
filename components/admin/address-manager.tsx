...
import { Plus, Pencil, Trash2, Loader2, Save, X, MapPin } from 'lucide-react'
...
export function AddressManager() {
  ...
  const handleSave = async (item: Partial<Address>) => {
    setSaving(true)
    try {
      if (item.id) {
        await supabase.from('addresses').update(item).eq('id', item.id)
      } else {
        await supabase.from('addresses').insert(item)
      }
      await fetchItems()
      setEditingItem(null)
      setIsCreating(false)
  ...
  const toggleActive = async (id: string, isActive: boolean) => {
    await supabase.from('addresses').update({ is_active: !isActive }).eq('id', id)
    await fetchItems()
  }
      ...
      <div className="space-y-4">
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-[#FFF1E8]">{item.city}</h3>
              <p className="text-sm text-[#FFF1E8]/60 truncate">{item.address}</p>
              {item.phone && (
                <p className="text-xs text-[#D39A6A]">{item.phone}</p>
              )}
            </div>
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-[#FFF1E8]/60">
            Aucune adresse. Cliquez sur &quot;Ajouter&quot; pour commencer.
          </div>
        )}
      </div>
    </div>
...
interface AddressFormProps {
  item: Address
  saving: boolean
  onSave: (item: Partial<Address>) => void
  onCancel: () => void
}

function AddressForm({ item, saving, onSave, onCancel }: AddressFormProps) {
  const [formData, setFormData] = useState(item)

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const dataToSave = { ...formData }
    if (!dataToSave.id) delete (dataToSave as Partial<Address>).id
    onSave(dataToSave)
  }
  ...
  return (
    <div className="bg-[#242424] rounded-xl p-6 border border-[#D39A6A]/30 mb-6">
      <h3 className="text-lg font-semibold text-[#FFF1E8] mb-4">
        {item.id ? 'Modifier l\'adresse' : 'Nouvelle adresse'}
      </h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label className="text-[#FFF1E8]/80">Ville</Label>
            <Input
              value={formData.city}
          ...
          <div className="space-y-2">
            <Label className="text-[#FFF1E8]/80">Téléphone</Label>
            <Input
        ...
        <div className="space-y-2">
          <Label className="text-[#FFF1E8]/80">Adresse complète</Label>
          <Input
